...
public static class PersistenceExtensions
{
    ...
    {
        services.AddDbContext<AdsDbContext>(options => options.UseNpgsql(connectionString, npgsql =>
                {
                    if (commandTimeout.HasValue)
                    {
                        npgsql.CommandTimeout(commandTimeout.Value);
                    }
                }))
                .AddDbContextFactory<AdsDbContext>((Action<DbContextOptionsBuilder>)null, ServiceLifetime.Scoped)
                .AddRepositories();

        services.AddScoped(typeof(IDistributedLock), _ => new PostgresDistributedLock(connectionString));
    ...
    public static IServiceCollection AddMultiTenantPersistence(this IServiceCollection services, Type connectionStringResolverType, Type tenantResolverType)
    {
        ...
        {
            return new PostgresDistributedLock(services.GetRequiredService<IConnectionStringResolver<AdsDbContextMultiTenant>>().ConnectionString);
        });
